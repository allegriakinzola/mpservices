  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      ...
      <div className={`flex flex-col gap-3 transition-all duration-300 ${isExpanded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
        {socialLinks.map((social) => (
          <a
            key={social.name}
            href={social.href}
            className={`w-12 h-12 bg-red-900/90 backdrop-blur-sm ${social.color} rounded-full flex items-center justify-center text-white shadow-lg transition-all duration-300 hover:scale-110 border border-white/20`}
            title={social.name}
            target="_blank"
      ...
      <button
        ...
        className={`w-14 h-14 bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-700 hover:to-orange-700 rounded-full flex items-center justify-center text-white shadow-xl transition-all duration-300 hover:scale-110 border border-white/20 ${isExpanded ? 'rotate-45' : ''}`}
